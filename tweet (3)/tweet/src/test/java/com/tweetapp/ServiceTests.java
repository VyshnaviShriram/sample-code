...
//	ProducerService producerService;
	...
	public void setup() {
		...
		tweet4 = new Tweet("16", "Name", user2, 1455L, LocalDateTime.now(), new ArrayList<>(), "Old");
	...
	public void testAddTweet() {
		tweet4 = new Tweet("36", "Lotus", user2, 1400L, LocalDateTime.now(),new ArrayList<>(), "@Flower");
		when(tweetRepo.save(tweet4)).thenReturn(tweet4);
		assertTrue(tweetService.addTweet("Karishma@1",tweet4).equals(tweet4));
	...
	public void testValidLikeTweet() throws Exception {
		when(tweetRepo.findById("12")).thenReturn(Optional.of(tweet1));
		assertThat(tweetService.likeTweet("12")).isEqualTo(tweet1.getLikes());
		verify(tweetRepo).findById("12");
	...
	public void testValidReplyTweet() throws Exception {
		when(tweetRepo.findById("12")).thenReturn(Optional.of(tweet1));
		when(userRepo.findById("Soumya@1")).thenReturn(Optional.of(user1));
		assertThat(tweetService.replyTweet("Soumya@1","12", replyTweet)).isEqualTo(tweet1);
		verify(tweetRepo).findById("12");
	}
	...
	public void testInvalidReplyTweet() throws Exception {
		when(tweetRepo.findById("38")).thenReturn(Optional.empty());
		assertThrows(TweetNotFoundException.class, () -> {
			tweetService.replyTweet("Karishma@1","38", tweet2);
		});
		verify(tweetRepo).findById("38");
	}
