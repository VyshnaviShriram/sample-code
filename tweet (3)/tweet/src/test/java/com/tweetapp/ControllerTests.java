...
public class ControllerTests {
	...
	public void setup() {
		...
		replyTweet = new Tweet("","",new User(),1L,LocalDateTime.now(),new ArrayList<>(),"Hi");
		response1 = new UserResponse(user1, "Login Success", "sbafkbHBAFJ1");
		loginUser = new UserLogin("Soumya@1","Soumya@9");
	}
	...
	public void testLogin() throws Exception {
		...
		this.mockMvc.perform(get("/api/v1.0/tweets/login").contentType(MediaType.APPLICATION_JSON)
				.content(new ObjectMapper().writeValueAsString(loginUser))).andExpect(status().isOk());
//		verify(userService, times(1)).login(loginUser.getUsername(), loginUser.getPassword());
	}
	...
	public void testForgotPassword() throws Exception {
		when(userService.resetPassword(any(), any())).thenReturn("Reset Password Successfully");
		this.mockMvc.perform(put("/api/v1.0/tweets/Soumya@1/forgot",loginUser).contentType(MediaType.APPLICATION_JSON)
				.content(new ObjectMapper().writeValueAsString(loginUser))).andExpect(status().isOk());
	...
	public void testValidSearchByUserName() throws Exception {
		...
		this.mockMvc.perform(get("/api/v1.0/tweets/user/search/@1")).andExpect(status().isOk());
	...
	public void testValidGetAllTweetsOfUser() throws Exception {
		...
		this.mockMvc.perform(get("/api/v1.0/tweets/{userName}", "Soumya@1")).andExpect(status().isOk());
	...
	public void testValidPostTweet() throws Exception {
		tweet3 = new Tweet("36", "Lotus", user2, 1400L, LocalDateTime.now(),new ArrayList<>(), "@Flower");
		when(tweetService.addTweet("Karishma@1",tweet3)).thenReturn(tweet3);
		...
		verify(tweetService, times(1)).addTweet(any(),any());
	}
	...
	public void testValidUpdateTweet() throws Exception {
		tweet3 = new Tweet("12", "Lotus", user1, 1400L, LocalDateTime.now(),new ArrayList<>(), "@Flower");
	...
	public void testValidLikeTweet() throws Exception {
		when(tweetService.likeTweet(any())).thenReturn(146L);
	...
	public void testValidReplyTweet() throws Exception {
		when(tweetService.replyTweet(any(),any(), any())).thenReturn(tweet2);
		this.mockMvc.perform(post("/api/v1.0/tweets/Karishma@1/reply/14").contentType(MediaType.APPLICATION_JSON)
				.content(new ObjectMapper().writeValueAsString(replyTweet))).andExpect(status().isOk());
		verify(tweetService, times(1)).replyTweet(any(),any(), any());
	}

	@Test
	@DisplayName("Test Invalid replyTweet()")
	public void testInvalidReplyTweet() throws Exception {
		when(tweetService.replyTweet(any(),any(), any())).thenThrow(new TweetNotFoundException());
		this.mockMvc.perform(post("/api/v1.0/tweets/Karishma/reply/15").contentType(MediaType.APPLICATION_JSON)
				.content(new ObjectMapper().writeValueAsString(replyTweet))).andExpect(status().isBadRequest());
//		verify(tweetService, times(1)).replyTweet(any(),any(), any());
	}
